...
def run_analysis(ae_shp: str, ze_shp: str, buffer_km: float = 5.0):
    ...
    :param buffer_km: distance de tampon autour de la zone d'étude en kilomètres
    """
    ...
    # Application du tampon autour de la zone d'étude
    buffer_dist = float(buffer_km) * 1000.0
    if buffer_dist > 0:
        try:
            reference2_gdf = gpd.GeoDataFrame(
                geometry=reference2_gdf.geometry.buffer(buffer_dist), crs=reference2_gdf.crs
            )
