...
def run_analysis(ae_shp: str, ze_shp: str, buffer_km: float = 5.0):
    ...
    :param buffer_km: distance du tampon appliqué autour de la zone d'étude (en km)
    """
    ...
    # Appliquer un tampon autour de la zone d'étude
    try:
        buffer_dist = buffer_km * 1000.0
        reference2_gdf = reference2_gdf.copy()
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        sys.exit(1)
    buffer = float(sys.argv[3]) if len(sys.argv) > 3 else 5.0
    run_analysis(sys.argv[1], sys.argv[2], buffer)
