# -*- coding: utf-8 -*-
"""Identification des zonages pour le contexte écologique.

Ce module contient une fonction principale `run_id_contexte_eco` qui reprend
le script fourni par l'utilisateur. Il produit un fichier Excel listant les
zonages intersectant ou proches de la zone d'étude.

Seule adaptation : les chemins vers les shapefiles de la zone d'étude et de
l'aire d'étude élargie sont passés en paramètres afin d'être choisis via
une boîte de dialogue dans l'interface graphique.
"""
...
def run_id_contexte_eco(couche_reference1: str, couche_reference2: str) -> None:
    """Lance le traitement d'identification des zonages."""
    ...
    except Exception as e:  # pragma: no cover - simple affichage
        log_with_time(
            f"Erreur lors du chargement de la première couche de référence : {e}"
        )
        return
    ...
    try:
        ...
        log_with_time(
            f"Erreur lors du chargement de la deuxième couche de référence : {e}"
        )
        return

    # S'assurer que les GeoDataFrames ont un CRS approprié
    ...
    if reference_gdf.crs != crs_projected:
        try:
            ...
            log_with_time(
                f"Erreur lors de la reprojection de la première couche de référence : {e}"
            )
            return
    ...
    if reference2_gdf.crs != crs_projected:
        try:
            ...
            log_with_time(
                f"Erreur lors de la reprojection de la deuxième couche de référence : {e}"
            )
            return
    ...
    try:
        ...
        log_with_time(
            f"Erreur lors du calcul du centroïde de la deuxième couche de référence : {e}"
        )
        return
    ...
    if os.path.exists(chemin_sortie):
        try:
            ...
            log_with_time(
                f"Le fichier existant '{chemin_sortie}' a été supprimé et sera remplacé par le nouvel export."
            )
        except Exception as e:
            log_with_time(
                f"Erreur lors de la suppression du fichier existant '{chemin_sortie}': {e}"
            )
            return
