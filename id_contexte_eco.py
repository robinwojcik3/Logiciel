...
def run_analysis(couche_reference1: str, couche_reference2: str, buffer_km: float = 5.0):
    ...
    buffer_km : float, optional
        Distance du tampon appliqué autour de la zone d'étude, en kilomètres.
        La valeur par défaut (5.0) reproduit le comportement historique du
        script lorsque ce paramètre n'était pas exposé.
    """
    ...
    # Appliquer un tampon optionnel autour de la zone d'étude
    try:
        buffer_dist = float(buffer_km) * 1000.0
        if buffer_dist > 0:
            reference2_gdf["geometry"] = reference2_gdf.buffer(buffer_dist)
            log_with_time(f"Tampon de {buffer_km} km appliqué autour de la zone d'étude")
    except Exception as e:
