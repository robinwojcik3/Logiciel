...
# Enregistrer directement dans OUTPUT (pas de sous-dossier PNG)
...
def worker_run(args: Tuple[List[str], dict]) -> Tuple[int, int]:
    """
    args[0] = liste de projets .qgz
    args[1] = config dict
    Retour: (ok_exports, ko_exports)
    """
    projects, cfg = args
    ...
    def relink_layer(prj: QgsProject, layer_name: str, shp_path: str) -> Optional[object]:
        """Relie la couche 'layer_name' à shp_path si trouvée. Retourne la couche ou None."""
        layers = prj.mapLayersByName(layer_name)
        if not layers:
            return None
        lyr = layers[0]
        try:
            # Shapefile -> provider "ogr"
    ...
    def export_views(projet_path: str) -> Tuple[int, int]:
        ...
        # Reliaison des sources vers les shapefiles choisis
...
def discover_projects() -> List[str]:
    ...
    # Tentative de parcours pour gérer les variations de casse (ex: "espace_rwo")
    try:
...
class ExportApp:
    def __init__(self, master):
        ...
        master.geometry("800x650") # Augmentation légère de la hauteur
    ...
    def _run_export_logic(self, projets):
        try:
            ...
            exports_per_project = 0
            if cadrage_mode == "BOTH": exports_per_project = 2
            elif cadrage_mode in ("AE", "ZE"): exports_per_project = 1
