# -*- coding: utf-8 -*-
"""Worker QGIS pour l'export des cartes.

Ce module isole la logique d'export afin que chaque sous-processus
puisse préparer correctement l'environnement QGIS/Qt sous Windows
avant tout import de ``qgis.*``.
"""
import os
import sys
import datetime
...
def log_with_time(msg: str) -> None:
    """Affiche un message horodaté comme dans l'UI principale."""
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] {msg}")


def to_long_unc(path: str) -> str:
    """Convertit un chemin en forme compatible UNC longue."""
    if path.startswith("\\\\?\\"):
        return path
    if path.startswith("\\\\"):
        return "\\\\?\\UNC" + path[1:]
...
def _prepare_qgis_env(cfg: Dict) -> str:
    ...
    Retourne le chemin de base Qt (qt_base) détecté.
    Requiert dans cfg: QGIS_ROOT, QGIS_APP, PY_VER (ex: 'Python312')
    """
...
# ---------- Helpers rapatriés depuis main_app.py ----------
...
def apply_extent_and_export(layout, lyr_extent: 'QgsRectangle', out_png: str, cfg: Dict) -> bool:
...
def export_views(projet_path: str, cfg: Dict) -> Tuple[int, int]:
    ...
    out_dir = cfg["OUT_DIR"]
    out_ae = os.path.join(out_dir, f"{nom}__AE.png")
    out_ze = os.path.join(out_dir, f"{nom}__ZE.png")
    out_proj = os.path.join(out_dir, f"{nom}{os.path.splitext(projet_path)[1]}")

    mode = cfg.get("CADRAGE_MODE", "BOTH")
    exp_type = cfg.get("EXPORT_TYPE", "PNG")
    expected_exports = 0
    if exp_type in ("PNG", "BOTH"):
        expected_exports += 2 if mode == "BOTH" else 1
    if exp_type in ("QGS", "BOTH"):
        expected_exports += 1
...
def test_qgis_import(cfg: Dict) -> None:
    """Simple import de QGIS pour valider l'environnement."""
    ...
    qgs.initQgis()
    try:
        log_with_time(f"QGIS import OK; QPA={os.environ.get('QT_QPA_PLATFORM')}; Qt={os.path.basename(qt_base)}")
    finally:
        qgs.exitQgis()


def worker_run(args: Tuple[List[str], Dict]) -> Tuple[int, int]:
    """Exécute l'export sur une liste de projets QGIS."""
    projects, cfg = args
    ...
    try:
        log_with_time(
            f"QGIS import OK; QPA={os.environ.get('QT_QPA_PLATFORM')}; Qt={os.path.basename(qt_base)}"
        )
        ok, ko = 0, 0
