...
def _log(msg: str) -> None:
    """Simple logger matching main app format."""
...
# ---------- Helpers déplacés depuis main_app.py ----------
...
def worker_run(args: Tuple[List[str], Dict]) -> Tuple[int, int]:
    projects, cfg = args
    _log(f"Worker start: {len(projects)} projets")
    ...
    _log(f"QGIS import OK; QPA={os.environ.get('QT_QPA_PLATFORM')}; Qt={os.path.basename(qt_base)}")
    ...
    try:
        ...
        ko = 0
        for p in projects:
            ok_c, ko_c = export_views(p, cfg)
            ok += ok_c
            ko += ko_c
        _log(f"Export OK/KO: {ok}/{ko}")
        return ok, ko
    finally:
        qgs.exitQgis()
...
def test_qgis_import(cfg: Dict) -> None:
    _prepare_qgis_env(cfg)
    from qgis.core import QgsApplication  # noqa: F401
    _log(f"QGIS import OK; QPA={os.environ.get('QT_QPA_PLATFORM')}")
