# -*- coding: utf-8 -*-
"""Fonctions d'export QGIS utilisées en multiprocess.
Ce module contient le code historique qui préparait correctement
l'environnement QGIS avant de lancer les exports. Il est séparé
pour éviter les erreurs de DLL lors du spawn des workers."""
...
def to_long_unc(path: str) -> str:
    """Normalise un chemin Windows en préfixe long UNC."""
    if path.startswith("\\\\?\\"):
        return path
    if path.startswith("\\\\"):
        return "\\\\?\\UNC" + path[1:]
...
def worker_run(args: Tuple[List[str], dict]) -> Tuple[int, int]:
    """Fonction exécutée dans chaque process pour exporter les cartes."""
    projects, cfg = args
    ...
    qpa = (
        ...
        if os.path.isfile(os.path.join(platform_dir, "qwindows.dll"))
            ...
            "minimal" if os.path.isfile(os.path.join(platform_dir, "qminimal.dll")) else "offscreen"
        )
    )
